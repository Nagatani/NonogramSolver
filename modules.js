...
let state = {
  rows: 5,       // 盤面の行数
  cols: 5,       // 盤面の列数
  rowRules: Array(5).fill([]), // 各行のルールを保持する2次元配列
  colRules: Array(5).fill([]), // 各列のルールを保持する2次元配列
  grid: Array(5).fill(null).map(() => Array(5).fill(CELL_STATE.EMPTY)), // 盤面の各セルの状態を保持する2次元配列
